...
// view engine setup
var db = mongojs('mongodb://', ['users','response','news','courses']);
...
{
   db.response.find({queid:"1"}).sort({response: -1},function(err, data) 
  {
        if (err) {
        throw err;
    } 
...
{
   db.response.find({ queid: "2"}).sort({response: -1},function(err, data) 
  {
        if (err) {
        throw err;
    } 
...
(gatherNode) => {
  gatherNode.say('Are you satisfied with the quality of food served at this university? press 1 if satisfied, press 2 if not');
});
...
db.response.insert( { response: request.body.Digits, queid: "2"  } );
